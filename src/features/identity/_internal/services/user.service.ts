...
export async function createUser(input: Actor & { email: string; name: string; roles: RoleAssignment[] }) {
  ...
  const tenantSmtp = await getTenantSmtpConfig(input.tenantId);
  const { delivered } = await sendMail({
    to: email,
    ...passwordSetupEmail("th", { name: input.name, link: setupLink(rawToken), hours: 72 }),
...
export async function issuePasswordSetupLink(input: Actor & { userId: string }) {
  ...
  const { delivered } = await sendMail({
    to: ut.user.email,
    ...passwordSetupEmail(asLocale(ut.user.locale), { name: ut.user.name, link: setupLink(raw), hours: 72 }),
    smtp: tenantSmtp ?? undefined,
  });
...
export async function requestEmailChange(input: Actor & { userId: string; newEmail: string }) {
  ...
  const { delivered } = await sendMail({
    to: newEmail,
    ...emailChangeEmail(asLocale(ut.user.locale), { name: ut.user.name, link: verifyLink(raw), hours: 24 }),
    smtp: tenantSmtp ?? undefined,
  });
  return { rawToken: raw, expiresAt, mailDelivered: delivered };
}
